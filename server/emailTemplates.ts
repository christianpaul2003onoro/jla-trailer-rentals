...
export function bookingReceivedHTML(
  ...
  const body = `
    ...
    <p style="margin-top:18px;font-weight:700;">Manage / check your booking:</p>
    <table role="presentation" style="font-size:14px;margin:10px 0;">
    ...
    <p style="font-size:14px;color:${brand.subtext}">
      We’ll review your details and send an approval with a payment link.
    </p>
  `;
  return layout({
...
// 5b) Cancelled
export function cancelledHTML(
  c: Common & { reason?: string | null }
) {
  const greeting = c.firstName ? `Hi ${c.firstName},` : "Hello,";
  const body = `
    <p>${greeting}</p>
